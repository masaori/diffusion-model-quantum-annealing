...
#note[
  ...
  - $1/x^r$ とかでなんかありそう
    - 次回(12/7)
    - $p(x)log(p(x))$ は $x -> infinity$ で $x -> 0$ だが、一度負に落ちて急速に0に向かう (#link("https://miro.com/app/board/uXjVKxaXwRQ=/?moveToWidget=3458764609755461490&cot=14")[miro])
    - 負の部分の面積が
        - p(X) の定数倍で抑えられる を示す => 有限
        - 無限大になるp(x) を具体的に作る
    - のどちらかをやってみる
    - 命題としては、1/f(x) の 単調増加関数 f(x) のクラス分けと捉える
      - f(x) := x p(x) 収束しない p(x)log(p(x)) 収束しない
      - f(x) := xlog(x) p(x) 収束しない p(x)log(p(x)) 収束しない
      - この間を探したい p(x) 収束する p(x)log(p(x)) 収束しない
      - f(x) := x^(1+ε) p(x) 収束する p(x)log(p(x)) 収束する
    - 実解析的 ⇔ テイラー展開可能
...
$
sum_(n >= 0) (x-1)^n dot.op binom(r,n) quad (|x-1| < 1)
$

== テイラー展開
f(x) : x = 0 の周りでテイラー展開すると
$
  f(x) &= c_1 + c_2 x + c_3 x^2 + c_4 x^3 + dots \
  f'(x) &= c_2 + 2 c_3 x + 3 c_4 x^2 + dots \
  f''(x) &= 2 c_3 + 6 c_4 x + dots \
  dots \
$
...
よって
$
  f(x) = f(0) + f'(0) x + f''(0) x^2 / 2 + dots
$

$
  f(x - a) = c_1 + c_2 (x - a) + c_3 (x - a)^2 + c_4 (x - a)^3 + dots \
  f'(x - a) = c_2 + 2 c_3 (x - a) + 3 c_4 (x - a)^2 + dots \
  f''(x - a) = 2 c_3 + 6 c_4 (x - a) + dots \
  dots \
$
$
  ...
  "よって" \
  f(x - a) = f(a) + f'(a) (x - a) + f''(a) (x - a)^2 / 2 + dots
  
$

$
(f(x) - c_1) / x　&= (c_1 + c_2 x + c_3 x^2 + c_4 x^3 + dots - c_1) / x \
&= c_2 + c_3 x + c_4 x^2 + dots \
$

$
  (f(x - a) - f(a)) / x &= (f(a) + f'(a) (x - a) + f''(a) (x - a)^2 / 2 + dots - f(a)) / x \
  &= f'(a) + f''(a) (x - a) / 2 + dots \
$

$
  exp(x) / x &= (1 + x + x^2 / 2 + x^3 / 6 + dots) / x \
  &= 1 + x / 2 + x^2 / 6 + dots  > 1 (x -> infinity)\
$
...
$
  forall epsilon > 0, exists M > 0 ((x > M) => |p(x)| < epsilon)
$

$epsilon = 1$ として、$(x > M) => |p(x)| < epsilon$ を満たすような $M > 0$を固定する。
...
$x=0$における、$f'(x)$と$(x log(x))'$の大きさを比較する

$(f'(x)) / (x log(x))' &= (f'(x)) / (log(x) + 1)$

$((f'(x)) / (log(x) + 1))' = (f''(x))/(log(x) + 1) - (f'(x)) / x(log(x) + 1)^2$

$((f'(x)) / (log(x) + 1))'' = (f'''(x))/(log(x)+1) - 2(f''(x))/(x(log(x)+1)^2) + (f'(x))/(x^2(log(x)+1)^2) + 2(f'(x))/(x^2(log(x)+1)^3)
$
